---
title: "ACx recordings with OFC inactivation - Analyses"
editor_options: 
...
# load('./Data/ACx_ephys_analyses_env.RData')
# 
# library(ggplot2)
# library(dplyr)
# library(tidyr)
...
df = read.csv('./Data/OFC-cannula_ACx-recording_allTrial_df.csv')
behavior_df = read.csv('./Data/OFC-cannula_ACx-recording_behavior_df.csv')
...
save.image('./Data/ACx_ephys_analyses_env.RData')
...
df_info = df[df$AMdepth_db != -40 & df$Period == 'Baseline',] %>% group_by(Subject, Unit, Subject_date, Cluster_quality, ActiveTrial_modulation_direction, Treatment) %>%
  summarise(
    Trial_count = length(unique(TrialID)),
    ISI_FPRate = mean(ISI_FPRate),
    ISI_ViolationRate = mean(ISI_ViolationRate),
...
vs_df = read.csv('./Data/OFC-cannula_ACx-recording_allTrial_VS.csv')
vs_df$Session = 'Pre'
